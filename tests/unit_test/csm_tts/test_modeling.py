...
PLAN §6.1. The depth-decoder greedy parity test vs HF pins the
position/offset/head off-by-ones (the #1 bug class here) and the §1.4
codebooks-head orientation (review F4) before any GPU work — hard M1-exit
blocker (PLAN §7 M1).
"""
...
# Real CSM-1B depth rope (config.json depth_decoder_config; PLAN §1.1) — used
...
def test_frame_embedding_matches_hand_rolled_reference() -> None:
    ...
    fp32, exact)."""
    fe = modeling.CsmFrameEmbedding(num_codebooks=32, codebook_vocab=2051, hidden_size=64)
...
def test_embed_codebook_offset_correctness() -> None:
    ...
    for every k in 0..31."""
    fe = modeling.CsmFrameEmbedding(num_codebooks=32, codebook_vocab=2051, hidden_size=8)
    _randomize(fe, std=0.02)
    codes = torch.randint(0, 2051, (4,))
    with torch.no_grad():
        for k in range(32):
            got = fe.embed_codebook(codes, k)
            assert torch.equal(got, fe.weight[codes + k * 2051]), f"offset wrong at k={k}"
...
def test_generate_frame_shapes_with_random_weights() -> None:
    ...
    (per-frame reset is structurally free, PLAN §2.2)."""
    cfg = _tiny_depth_cfg(num_codebooks=32, vocab_size=51)
    fe = modeling.CsmFrameEmbedding(cfg.num_codebooks, cfg.vocab_size, cfg.backbone_hidden_size)
...
def test_depth_decoder_greedy_parity_vs_hf() -> None:
    ...
    §1.4 head orientation + the position/offset scheme + our llama3-rope port.
    ...
    # Copy OUR random weights into the HF reference (§1.6 layer mapping).
    state = {
    ...
    for i, layer in enumerate(dd.layers):
        ...
        state[pre + "post_attention_layernorm.weight"] = layer.post_attention_layernorm.weight
