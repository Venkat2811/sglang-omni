...
"""Unit tests for csm_tts.sampler (CPU, no GPU; PLAN §6.1) plus the
frame-0-EOS zero-decode lifecycle (F7) at the runner level (sglang stubbed
...
def test_eos_uses_cb0_to_cb30_only() -> None:
    ...
    # (HF ``sequences`` parity; the vocoder never receives it, PLAN §2.5).
...
def test_sampler_state_pool_reset_row_and_staging_width() -> None:
    ...
    width contract (PLAN §2.6: ``c0..c31 | was_done | generation_done``)."""
...
def test_forced_frame0_eos_zero_decode_lifecycle() -> None:
    """F7 (PLAN §1.7 / §2.4): a frame-0 EOS sampled AT PREFILL finishes the
    ...
    fake_model = SimpleNamespace(
        ...
        _sampler_pool=SimpleNamespace(
            generation_done=torch.tensor([True, False])
        ),
    )
