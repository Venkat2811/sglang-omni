...
def test_registry_smoke() -> None:
    ...
    (contract §1; a silent registry skip would fail this)."""
...
def test_config_topology() -> None:
    ...
    (schema demands exactly one of next/terminal per stage — review B11)."""
...
class _CharTokenizer:
    ...
    def encode(self, text: str, add_special_tokens: bool = True) -> list[int]:
        assert add_special_tokens is False  # PLAN §1.9 hard requirement
...
def test_prompt_builder_layout_matches_hub_chat_template() -> None:
    ...
    against the real checkpoint tokenizer are part of the M1 container gate —
    ...
    expected = (
        [BOS] + seg0 + [EOT] + [AUDIO_TOKEN_ID] * 3 + [AUDIO_EOS_TOKEN_ID]
        + [BOS] + seg1 + [EOT] + [AUDIO_TOKEN_ID] * 2 + [AUDIO_EOS_TOKEN_ID]
        + [BOS] + final + [EOT]
    )
    assert ids == expected
    assert spans == [
...
class _MockMimi:
    ...
    def decode(self, codes_F32: torch.Tensor) -> torch.Tensor:
        ...
        assert int(codes_F32.min()) >= 0 and int(codes_F32.max()) <= 2047, (
...
def test_vocoder_framing_math_default_strides() -> None:
    """Delay-free framing at 12.5 Hz (PLAN §4.1): first chunk at stride=13
...
def test_vocoder_trim_and_clamp_fences() -> None:
    """Fences #2/#3 (PLAN §4.2): the final flush trims at the first
...
def test_prefill_overlay_paste_positions() -> None:
    ...
    prompt = [BOS, 200_065, EOT] + [AUDIO_TOKEN_ID] * 3 + [AUDIO_EOS_TOKEN_ID] + [BOS, 200_066, EOT]
    ...
    assert torch.equal(
        embeds[text_positions], embed_tokens(input_ids[text_positions])
    )
...
def test_prefill_overlay_chunked_cursor() -> None:
    ...
    sequentially; overrunning the codes raises (PLAN §8.R3 off-by-one
    class)."""
    ...
    chunk2 = torch.tensor(
        [AUDIO_TOKEN_ID] * 2 + [AUDIO_EOS_TOKEN_ID], dtype=torch.long
    )
...
@pytest.mark.skip(
    reason="needs sglang + torchaudio (stages.py engine import chain); the "
    "factory recipe (context_length=2048, disable_overlap_schedule, mrr 8, "
    "chunked_prefill 2048, bf16, no truncate_rope_to_bf16, on_retract abort) "
    "is validated by the M1 container gate (PLAN §6.1/§7)"
)
def test_engine_factory_defaults() -> None:
    raise NotImplementedError


@pytest.mark.skip(
    reason="prefill-path finish marking (frame-0 EOS, F7) is covered by "
    "test_sampler.test_forced_frame0_eos_zero_decode_lifecycle; the decode-"
    "path variant needs the sglang ScheduleBatch machinery (container M2)"
)
def test_runner_finish_marking_on_synthetic_model() -> None:
    raise NotImplementedError
