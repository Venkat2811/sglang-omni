...
def _state(prompt_len: int = 8, **kwargs) -> CsmTtsState:
    return CsmTtsState(
        text="hi", prompt_ids=list(range(1, prompt_len + 1)), **kwargs
    )


def _payload(state: CsmTtsState, rid: str = "req-1", params: dict | None = None) -> StagePayload:
    return StagePayload(
        request_id=rid,
...
def test_max_new_tokens_clamp_vs_ctx_budget() -> None:
    ...
    max_req_len = 2047, NOT 2048 (review A4/B18); zero budget raises an
    actionable error."""
...
def test_context_fingerprint_sensitivity() -> None:
    ...
    assert (
        request_builders._context_fingerprint(state_for([codes], speaker=1))
        != base_fp
    )
