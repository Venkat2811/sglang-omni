...
# shards `transformers-0000x-of-00002.safetensors`) — PLAN §8.R2.
...
def trim_at_first_all_zero_frame(codes_F32: torch.Tensor) -> torch.Tensor:
    """Fence #3 of PLAN §4.2: cut at the first frame whose ALL 32 codebooks
    ...
    for shard, names in shards.items():
        ...
        with safe_open(shard_path, framework="pt") as f:
            ...
            for full_name in keys:
                state[full_name[len(_CODEC_IN_CKPT_PREFIX) :]] = f.get_tensor(
                    full_name
                )
    return state
...
class CsmMimiCodec:
    ...
    def __init__(self, model: Any, device: str, dtype: torch.dtype) -> None:
        ...
        # tracks per-request counts. Gate-able only at temp=0 (F11).
        self.clamp_count = 0
    ...
    def _exact_frames(self, wave: torch.Tensor, num_frames: int) -> torch.Tensor:
        ...
        relies on (Mimi full decode is exact; spec §5)."""
        expected = num_frames * self.SAMPLES_PER_FRAME
        if wave.shape[-1] > expected:
