...
# Three-tier context-audio caching (higgs copy, PLAN §1.14): path-hash memo /
...
def _normalize_context_inputs(inputs: dict[str, Any], default_speaker: int) -> list[dict[str, Any]]:
    ...
    *,
    ...
    def _preprocess(payload: StagePayload) -> StagePayload:
        ...
        for seg in _normalize_context_inputs(inputs, speaker_id):
            seg_speaker = int(seg.get("speaker", seg.get("speaker_id", speaker_id)) or 0)
            ...
            if codes is not None:
                ...
                # the placeholder count comes from the ACTUAL encode (F14).
        ...
        if has_raw_audio:
            ...
            prompt_ids, _spans = builder.build_prompt(
                ...
                context_frame_counts=[int(c.shape[0]) for c in context_codes if c is not None],
            )
        ...
        state = CsmTtsState(
            ...
            max_new_tokens=int(max_new_tokens) if max_new_tokens else DEFAULT_MAX_FRAMES,
...
def create_audio_encoder_executor(
    ...
    *,
    ...
    def _encode(payload: StagePayload) -> StagePayload:
        ...
        # pre-encoded segments) — never the closed-form estimate (F14).
        prompt_ids, _spans = builder.build_prompt(
            text=state.text or "",
...
def create_sglang_tts_engine_executor(
    ...
    *,
    ...
    overrides: dict[str, Any] = {
        # M1-M3 run eager (correctness baseline); flips to False in M4 with
        # the captured backbone+cb0+depth step (PLAN §7, §8.R4).
    ...
    # bf16-training-parity rationale doesn't apply (PLAN §1.14).

    def _abort_on_retract(req: Any) -> None:
        # PLAN §8.R1a: retraction would replay the prompt and sample a NEW
        ...
        raise RuntimeError(
            ...
            "stream has no rollback (PLAN §8.R1a). Retraction means the KV "
