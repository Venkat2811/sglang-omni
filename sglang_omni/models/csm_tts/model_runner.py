...
(PLAN §2.5). Async stays OFF until M4 (``enable_async_decode=False`` in the
...
class CsmTTSModelRunner(ModelRunner):
            ...
            if n_placeholders > 0:
                ...
                if codes is None:
                    raise ValueError(
                        ...
                        f"context_codes (prompt/codes mismatch — PLAN §8.R3)"
                    )
                ...
                if consumed + n_placeholders > codes.shape[0]:
                    raise ValueError(
                        ...
                        f"PLAN §8.R3 off-by-one class)"
                    )
                with torch.no_grad():
                    paste = frame_embedding(
    ...
    def _populate_cg_buffers(
        ...
        captured region) — PLAN §2.1 step 1.
        ...
        # ``req._omni_data`` (= sched_req.data; PLAN §2.1 step 1).
        depth_temps, depth_top_ks = self._extract_depth_sampling_params(requests)
    ...
    def _collect_step_outputs(self, result: Any, requests: list) -> None:
        ...
        (PLAN §1.7 / F7 — finished at prefill, nothing streamed); overwrites
    ...
    # --- decode (async halves; implemented but OFF until M4) --------------------
    ...
    def post_decode_launch(self, result: Any, forward_batch: Any, requests: list) -> Any:
        ...
        PLAN §2.6). Dormant until M4 (``enable_async_decode=False``).
