...
_EXACT_MAP: dict[str, str] = {
    ...
    # word embeddings precisely so no text lm_head exists to mis-load, R2).
...
_SKIP_EXACT: dict[str, str] = {
    ...
    # so this skip ALWAYS fires for the ship artifact (F13).
...
class CsmWeightMapper:
    ...
    def _route(self, name: str) -> tuple[str, str | None]:
        ...
        if name.startswith(_BACKBONE_LAYERS_PREFIX):
            return "mapped", "backbone.model.layers." + name[len(_BACKBONE_LAYERS_PREFIX) :]
    ...
    def map(self, name: str) -> str | None:
        ...
        Raises:
            ValueError: on any tensor name outside the known census — strict
                accounting; never silently drop or misroute (PLAN §1.6, R2).
        """
        kind, dest = self._route(name)
        if kind == "unexpected":
            raise ValueError(
                f"Unexpected checkpoint tensor {name!r}: not in the CSM remap "
                f"table and not a deliberate skip (PLAN §1.6 strict census)"
            )
        return dest
