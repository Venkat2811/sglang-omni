...
class CsmTTSModel(nn.Module):
    ...
    def __init__(
        ...
        # Packed D2H row: c0..c31 | was_done | generation_done (PLAN §2.6).
    ...
    def decode_codebooks_batch_cg(self, hidden_BD: torch.Tensor) -> torch.Tensor:
        ...
        M4 TODO: capture this whole tail (together with the backbone forward)
        as one CUDA graph per batch size — eager-only for M2 (PLAN §8.R4).
    ...
    def _extract_batch_metadata(
        ...
        # ``req._omni_data`` in before_prefill (PLAN §2.1 step 1).
        depth_params = getattr(forward_batch, "csm_depth_params", None)
        gen_params = self._gen_params_for_batch(
            sampling_info, depth_params, batch_size
        )
    ...
    def load_weights(self, weights: Iterable[Tuple[str, torch.Tensor]]) -> set[str]:
        ...
        mapper raises on any tensor outside the known census (M1 strict
