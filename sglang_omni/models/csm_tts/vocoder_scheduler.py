...
class CsmStreamingVocoderScheduler(StreamingSimpleScheduler):
    ...
    def on_stream_done(self, request_id: str) -> list[OutgoingMessage]:
        ...
        if not messages and state.frames_emitted == 0:
            ...
            # one 80 ms frame when cb31 != 0 (PLAN §2.5 deviation note).
        ...
        if state.clamp_count:
            # Fence-#2 telemetry; parity-gate asserts this stays absent/0 at
            # temp=0 (PLAN §6.2 / F11).
            final_data["mimi_clamp_count"] = state.clamp_count
        messages.append(
            OutgoingMessage(
                request_id=request_id,
                type="result",
        ...
        if final:
            ...
            if not has_emitted:
                ...
                emit_until = (
                    initial
                    if use_initial
                    else available - self._stream_holdback_frames
                )
            else:
                need = (
                    state.frames_emitted
    ...
    def _frames_to_codes(frames: list[Any] | None) -> torch.Tensor | None:
        ...
        if not frames:
            return None
        rows = [
            torch.as_tensor(row, dtype=torch.long).reshape(-1) for row in frames
        ]
